import { AVAILABLE_ENDPOINTS, fal } from "../../lib/videosos/fal";
import { getRunwareClient, prepareRunwareImageAsset } from "../../lib/videosos/runware";
import { RUNWARE_ENDPOINTS } from "../../lib/videosos/runware-models";
import { downloadUrlAsBlob } from "../../lib/videosos/utils";
...
export const useJobCreator = ({
  ...
  const prepareFalInput = async (
    ...
    if (endpoint?.inputMap) {
      const { mapInputKey } = await import("../../lib/videosos/utils");
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      if (mediaType === "image") {
        const { buildRunwarePayload } = await import("../../lib/videosos/runware-capabilities");
        ...
        (async () => {
          ...
          if (hasBlobOrFileImage && blobOrFileImageValue) {
            ...
            const { getModelCapabilities } = await import("../../lib/videosos/runware-capabilities");
