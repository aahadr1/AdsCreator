...
export async function POST(req: NextRequest) {
  try {
    ...
    const adArchiveId = body?.adArchiveId;
    const pageId = body?.pageId;
    if (!adArchiveId || !pageId) return new Response('Missing adArchiveId or pageId', { status: 400 });

    const projectRoot = process.cwd();
    const scriptPath = path.join(projectRoot, 'vendor', 'fb_ad_scraper', 'api_wrapper.py');

    const payload = JSON.stringify({ mode: 'adsdetail', adArchiveId, pageId });
