...
async function runLabeling({ url, kind, replicate }: { url: string; kind: MediaKind; replicate: Replicate }): Promise<LabelResult> {
  ...
  const llmOut = (await replicate.run(model as `${string}/${string}`, {
...
export async function POST(req: NextRequest) {
  try {
    ...
    type AssetRecord = { id: string; public_url: string | null; storage_path: string; kind: MediaKind };
    ...
    if (assetId) {
      ...
      asset = (data as AssetRecord) || null;
    } else if (storagePath) {
